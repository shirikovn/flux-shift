...
def main(config: DictConfig) -> None:
    ...
    if not isinstance(benchmark_config, dict):
        raise TypeError(
            "benchmark configuration must resolve to a mapping."
        )

    raw_concepts = benchmark_config["concepts"]
    raw_seeds = benchmark_config["seeds"]

    if not isinstance(raw_concepts, list):
        raise TypeError("benchmark.concepts must be a list.")

    if not isinstance(raw_seeds, list):
        raise TypeError("benchmark.seeds must be a list.")
    ...
    task = resolve_task(
        task_id=int(benchmark_config["task_id"]),
        concepts=concepts,
        seeds=seeds,
    )
    ...
    cases = build_cases(
        ...
        num_templates=int(
            benchmark_config["num_templates"]
        ),
    )

    # Make the selected seed/cases part of the resolved
    # configuration and therefore of the run manifest.
    with open_dict(config):
        config.seed = task.seed
        config.experiment.cases = OmegaConf.create(cases)

    logger.info(
        "Benchmark task:\n%s",
        OmegaConf.to_yaml(
            OmegaConf.create(asdict(task)),
            resolve=True,
        ),
    )
    ...
    run_name = (
        f"snoopy_benchmark_"
        f"{task.concept_key}_seed_{task.seed}"
    )
    ...
    with RunManifest(
        ...
        with manifest.stage("snoopy_benchmark"):
