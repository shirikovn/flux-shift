...
class BlockwiseSVMRegularizer:
    ...
    def __init__(
        ...
        self.classifier_directory = Path(
        ...
        if not self.classifier_directory.is_dir():
            raise FileNotFoundError(
                "Classifier directory does not exist: "
                f"{self.classifier_directory}"
            )

        if not self.block_indices:
            raise ValueError(
                "At least one classifier block is required."
            )
        ...
        for block_index in self.block_indices:
            ...
            if not path.is_file():
                raise FileNotFoundError(
                    f"Classifier for block {block_index} "
            ...
            try:
                ...
                raise RuntimeError(
                    "Positive class label 1 is absent "
                    f"for block {block_index}: {classes}"
                ) from error
    ...
    def _get_classes(
        ...
        if classes is None:
            raise RuntimeError(
                "Could not find classifier classes_."
            )
    ...
    def predict(
        ...
        if block_index not in self._classifiers:
            raise KeyError(
                f"No classifier loaded for block "
                f"{block_index}."
            )
        ...
        probabilities = (
            classifier.predict_proba(pooled)
        )
